#' VLM Setup Helpers
#'
#' Functions to help users set up VLM dependencies for tidylens.
#'
#' @name vlm_setup
#' @keywords internal
NULL
...
#'
#' @param base_url Base URL for Ollama server. Default
...
#' @family vlm
#' @export
vlm_check_ollama <- function(base_url = "http://localhost:11434", verbose = TRUE) {
  tryCatch({
    ...
    if (verbose) {
      ...
      cli::cli_alert_info("Start Ollama or see {.code ?vlm_check_ollama} for installation instructions")
    }
...
#'
#' Lists vision-capable models that can be used with tidylens VLM functions.
#'
#' @param base_url Base URL for Ollama server. Default
#'   `"http://localhost:11434"`.
...
#' \dontrun{
#' vlm_list_models()
#' vlm_list_models(only_installed = TRUE)
#' }
#'
#' @family vlm
#' @export
vlm_list_models <- function(base_url = "http://localhost:11434",
  ...
  if (only_installed || vlm_check_ollama(base_url, verbose = FALSE)) {
    tryCatch({
      response <- httr2::request(base_url) |>
      ...
      if (httr2::resp_status(response) == 200) {
        ...
        if (length(data$models) > 0) {
          ...
          if (only_installed) {
            cli::cli_alert_warning("No vision models installed. Use {.code vlm_pull_model()} to install.")
            return(NULL)
          }
        }
      }
...
#'
#' Downloads a vision-capable model for use with tidylens VLM functions.
#'
...
#' @examples
#' \dontrun{
#' vlm_pull_model("moondream")
#' vlm_pull_model("qwen2.5vl:7b")
#' vlm_pull_model("llama3.2-vision")
#' }
#'
#' @family vlm
#' @export
vlm_pull_model <- function(model = "qwen2.5vl:7b",
                           base_url = "http://localhost:11434") {

  if (!vlm_check_ollama(base_url, verbose = FALSE)) {
...
#' Check VLM dependencies for tidylens
#'
#' Verifies all required packages and services for VLM functions.
#'
...
#' @details
#' ## Required R Packages
#' - httr2: HTTP requests to the Ollama VLM API.
#' - base64enc: Encoding images for API calls.
#' - jsonlite: Parsing API responses.
#'
#' ## Ollama (Local VLM)
#' 1. Install Ollama (see [vlm_check_ollama()]).
#' 2. Start Ollama.
#' 3. Pull a vision model: `vlm_pull_model("qwen2.5vl:7b")`.
#'
#' @examples
#' \dontrun{
...
#' @family vlm
#' @export
vlm_check_dependencies <- function(verbose = TRUE) {
  ...
  status$ollama$running <- vlm_check_ollama(verbose = verbose)

  if (status$ollama$running) {
    models <- vlm_list_models(only_installed = TRUE)
    if (!is.null(models) && nrow(models) > 0) {
      ...
      if (verbose) {
        cli::cli_alert_warning("No vision models installed. Run: vlm_pull_model(\"qwen2.5vl:7b\")")
      }
    }
  }
...
#' Print setup instructions for VLM functions
#'
#' Displays comprehensive setup instructions for all supported platforms.
#'
#' @param platform Operating system: `"auto"`, `"macos"`, `"windows"`, or
#'   `"linux"`.
#'
...
#' @family vlm
#' @export
vlm_setup_instructions <- function(platform = "auto") {
  ...
  cli::cli_text("In terminal (or R with vlm_pull_model()):")
  ...
  cli::cli_code('
library(tidylens)
vlm_check_ollama()       # Should show Ollama is running
vlm_list_models()        # List available models
  ')

  cli::cli_h2("Step 5: Use VLM Functions")
  cli::cli_code('
images <- load_images("my_images/")
images |>
  vlm_describe(model = "qwen2.5vl:7b") |>
  vlm_classify(categories = c("action", "dialogue", "landscape"))
  ')
