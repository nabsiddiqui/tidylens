...
```r
# Describe images
images <- vlm_describe(images, model = "qwen2.5vl:7b")
# Adds: vlm_description column

# Classify into categories
images <- vlm_classify(images, 
                       categories = c("indoor", "outdoor", "portrait", "landscape"))
# Adds: vlm_category column

# Analyze sentiment/mood
images <- vlm_sentiment(images)
# Adds: vlm_mood, vlm_mood_valence, vlm_mood_intensity columns

# Object recognition
images <- vlm_recognize(images)
# Adds: vlm_objects, vlm_people_count, vlm_text_detected columns
```
...
| Function | Description |
|----------|-------------|
| `vlm_describe()` | Natural language descriptions |
| `vlm_classify()` | Category classification |
| `vlm_sentiment()` | Mood/sentiment analysis |
| `vlm_recognize()` | Object recognition |
| `vlm_scale()` | VLM-based shot scale classification |
| `vlm_check_ollama()` | Verify Ollama is running |
| `vlm_list_models()` | List recommended models |
| `vlm_pull_model()` | Download a model |
| `vlm_check_dependencies()` | Verify all VLM requirements |
